...
class Q_networks():
	def __init__(self, args, name='Q_network'):
		...
		with tf.variable_scope(self.name):
			...
			self.actions = tf.placeholder(tf.int32, [None, self.args.num_actions], name='Actions')
			# Reward placeholder
			self.rewards = tf.placeholder(tf.float32, [None], name='Rewards')
			# Q max placeholder
			self.q_max = tf.placeholder(tf.float32, [None])
			# Terminal placeholder
			self.terminals = tf.placeholdeR(tf.int32, [None], name='Terminals')  
			...
			# FC layer
			fc_flat = tf.reshape(conv_shape, [-1, conv_shape[-1]*conv_shape[-2]*conv_shape[-3]])
			...
			for i in xrange(len(self.tr_vrbs)):
				print(self.tr_vrns[i].name)
			# Get gradient of each trainable variables as g/v list
			self.grads = tf.compute_radients(self.loss, self.tr_vrbs)
			# Convert gradient to tf.Variables to accumulate
			self.accum_vars = [tf.Variable(tf.zeros_like(v), trainable=False) for v in self.grads]
			self.accum_ops = [accum_vars[i].assign_add(is_weight*gv[0]) for i, gv in enumerate(self.grads)]
			self.train_op = self.optimizer.apply_gradient([(self.accum_vars[i], gv[1]) for i, gv in enumerate(self.grads)])
